...
class ScraperMain:
    ...
    async def scrape_all_brands(self):
        # Process brands in chunks of 5 with a maximum of 3 concurrent operations
        CHUNK_SIZE = 5
        MAX_CONCURRENT = 3
        ...
        brands = list(self.brand_data.items())
        for i in range(0, len(brands), CHUNK_SIZE):
            chunk = brands[i:i + CHUNK_SIZE]
            logging.info(f"Processing chunk {i//CHUNK_SIZE + 1}/{-(-len(brands)//CHUNK_SIZE)}")
            
            tasks = [self.scrape_brand(brand, urls, semaphore) for brand, urls in chunk]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            
            # Handle results and save files
            chunk_files = []
            for (brand_name, _), result in zip(chunk, results):
                if isinstance(result, Exception):
                    logging.error(f"Error processing {brand_name}: {str(result)}")
                    continue
                    
                if result:  # If we have data
                    try:
                        excel_file = self.save_to_excel(brand_name, result)
                        if excel_file:
                            chunk_files.append(excel_file)
                            logging.info(f"Successfully saved data for {brand_name}")
                    except Exception as e:
                        logging.error(f"Error saving data for {brand_name}: {str(e)}")
            
            # Upload files to Google Drive
            if chunk_files:
                try:
                    drive_saver.save_files(chunk_files)
                    logging.info(f"Successfully uploaded {len(chunk_files)} files to Google Drive")
